'use strict';
// throws error when neede to keep js tight

(function(module) {
  // function that applies to the whole window
  function Article (opts) {
    ...
    Object.keys(opts).forEach(function(e, index, keys) { //object.keys returns array of all keys
      this[e] = opts[e]; //this is
    },this);
  }
  ...
  // DONE: Set up a DB table for articles. **255 is a memory unit
  Article.createTable = function(callback) {
    webDB.execute('CREATE TABLE [ARTICLES]('+
  'id INTERGER PRIMARY KEY,'+
  'author VARCHAR(100) NOT NULL,'+
  'authorUrl VARCHAR(100) NOT NULL,'+
  'title VARCHAR(100) NOT NULL,'+
  'category VARCHAR(100) NOT NULL,'+
  'body VARCHAR(10000) NOT NULL NOT BLANK,'+
...
// what SQL command do we run here inside these quotes?
  ...
  Article.truncateTable = function(callback) { //truncateTable deletes whats in the table but not the table
